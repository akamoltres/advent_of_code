...
static long run_amplifiers(int program_length, const int bufsize, long *program,
                           long phase_settings[NUM_AMPS])
{
    int previous_output = 0;

    for(int amp = 0; amp < NUM_AMPS; ++amp)
    {
        long amp_program[bufsize];
        memcpy(amp_program, program, bufsize * sizeof(long));
...
static long try_phase_settings(int depth, long phase_settings[NUM_AMPS],
                               int program_length, const int bufsize, long *program)
{
    if(depth == NUM_AMPS)
    {
    ...
    for(int i = 0; i < NUM_AMPS; ++i)
    {
        ...
        if(!already_used)
        {
            ...
            long max_with_phase = try_phase_settings(depth + 1, phase_settings,
...
static long run_amplifiers_loop(int program_length, const int bufsize, long *program,
                                long phase_settings[NUM_AMPS])
{
    ...
    long amp_programs[NUM_AMPS][bufsize];
    long amp_pc[NUM_AMPS] = {0};
    for(int amp = 0; amp < NUM_AMPS; ++amp)
    {
        memcpy(amp_programs[amp], program, bufsize * sizeof(long));
    }
...
static long try_phase_settings_loop(int depth, long phase_settings[NUM_AMPS],
                                    int program_length, const int bufsize, long *program)
{
    if(depth == NUM_AMPS)
    {
    ...
    for(int i = LOOP_OFFSET; i < LOOP_OFFSET + NUM_AMPS; ++i)
    {
        ...
        {
            phase_settings[depth] = i;
            long max_with_phase = try_phase_settings_loop(depth + 1, phase_settings,
                                                         program_length, bufsize, program);
            if(max_with_phase > max_signal)
            {
                max_signal = max_with_phase;
