...
int read_input_1(char *input_filename, CharPair_t ***buffer)
{
...
int read_input_2(char *input_filename, CharPair_t ****buffer)
{
    FILE *fp = fopen(input_filename, "r");
    if(fp == NULL)
    {
    ...
    while(fgets(c, 2, fp) != NULL)
    {
        ...
        assert(row < BUFSIZE);
        assert(col + 1 < BUFSIZE);
        buffer[row][col++][BUFSIZE / 2][BUFSIZE / 2].a = c[0];
    }
...
void free_buffer_1(CharPair_t ***buffer)
{
...
void free_buffer_2(CharPair_t ****buffer)
{
    for(int i = 0; i < BUFSIZE; ++i)
    {
        for(int j = 0; j < BUFSIZE; ++j)
...
int part1(char *input_filename)
{
    ...
    if(read_input_1(input_filename, buffer))
    {
        return -1;
    }
...
int part2(char *input_filename)
{
    ...
    for(int i = 0; i < BUFSIZE; ++i)
    {
        ...
        for(int j = 0; j < BUFSIZE; ++j)
        {
            ...
            for(int k = 0; k < BUFSIZE; ++k)
            {
                buffer[i][j][k] = (CharPair_t *) malloc(BUFSIZE * sizeof(CharPair_t));
                for(int l = 0; l < BUFSIZE; ++l)
                {
                    buffer[i][j][k][l].a = buffer[i][j][k][l].b = '.';
                }
            }
        }
    }
    ...
    // get the initial state
    if(read_input_2(input_filename, buffer))
    {
        return -1;
    }
                            ...
                            for(int b = -1; b <= 1; ++b)
                            {
                                for(int c = -1; c <= 1; ++c)
                                {
                                    for(int d = -1; d <= 1; ++d)
                                    {
                                        ...
                                        active_neighbors += (buffer[i + a][j + b][k + c][l + d].a == '#');
                                    }
                                }
                            }
                        }
